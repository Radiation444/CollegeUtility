...
import 'chat_room_screen.dart'; // IMPORT THE UNIVERSAL CHAT ROOM!
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void _showEditProfileSheet(Map<String, dynamic> userData) {
    ...
    String selectedDept = userData['department'] ?? _departments.first;

    String selectedHostel = userData['hostel'] ?? _hostels.first;
    ...
    return Scaffold(
      ...
      body: FutureBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          String displayName = userData['name'] ?? email.split('@').first;
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              children: [
                  ...
                  child: Stack(
                    children: [
                      CircleAvatar(
                        ...
                        child: imageUrl == null && _image == null
                            ? Text(
                ...
                // --- UNIVERSAL CHAT BUTTON INTEGRATION ---
                if (!isOwnProfile)
                  ElevatedButton.icon(
                    onPressed: () async {
                      ...
                      if (myUserId == null || myUserId == profileUserId) return;

                      // 1. Create the exact same unique Chat ID
                      List<String> users = [myUserId, profileUserId];
                      users.sort();
                      String chatId = "${users[0]}_${users[1]}";

                      // 2. Safely create the chat room IF it doesn't exist yet
                      await FirebaseFirestore.instance.collection('chats').doc(chatId).set({
                        'participants': [myUserId, profileUserId],
                      }, SetOptions(merge: true));

                      // 3. Jump straight into the Chat Room!
                      if (context.mounted) {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                    ...
                    child: Column(
                      children: [
                        ...
                        ListTile(
                          ...
                          subtitle: Text(userData['department'] ?? "N/A"),
                        ),
                        const Divider(),
                        ListTile(
                          leading: const Icon(Icons.home),
                          title: const Text("Hostel"),
                          subtitle: Text(userData['hostel'] ?? "N/A"),
                        ),
                        const Divider(),
                        ListTile(
